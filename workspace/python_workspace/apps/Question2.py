...
# 1 Renaming columns
df = df.withColumnRenamed('HomeTeam', 'Home_Team')
df = df.withColumnRenamed('HomeGoals', 'Home_Team_Goals')
df = df.withColumnRenamed('AwayGoals', 'Away_Team_Goals')
df = df.withColumnRenamed('AwayTeam', 'Away_Team')
...
df.filter(df.Round.like("Semi-finals") & (df.Home_Team == 'Argentina')).count()
# 6 Display all the finale matches
df.filter(df.Round == 'Final').show()
# 7 pairing stage wise matches with home team
df.crosstab('Round', 'Home_Team').show()
# 8 finding mean, max, min etc of the attendance over the entire matches
df.describe(['Away_Team_Goals']).show()
...
if __name__ == "__main__":
    ...
    lines = sc.textFile("DataSet/WorldCups.csv", 1)
    header = lines.first()
